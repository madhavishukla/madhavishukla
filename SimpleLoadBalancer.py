...
class SimpleLoadBalancer(object):
    ...
    def update_lb_mapping(self, client_ip): 
        ...
        #log.info("list of servers" + str(self.server_ips))
        if self.flow_table.get(client_ip, (None, None)) == (None, None):
            chosen_server = random.choice(self.server_ips)
            self.flow_table[client_ip] = chosen_server, current_time
        elif abs(self.flow_table[client_ip][1] - current_time) <= FLOW_TIMEOUT:
            #log.info("time difference :" + str(abs(self.flow_table[client_ip][1]
            #log.info("time difference :" + str(abs(self.flow_table[client_ip][1] - current_time)))
            self.flow_table[client_ip] = (self.flow_table[client_ip][0],current_time)
        else:
            #log.info("time difference :" + str(abs(self.flow_table[client_ip][1] - current_time)))
            chosen_server = random.choice(self.server_ips)
            self.flow_table[client_ip] = chosen_server, current_time
            #log.info("our flow table : " + str(client_ip) + " and server is : " + str(self.flow_table[client_ip][0]))
    ...
    def forward_client_to_server(self, event, client_ip):
        ### write your code here ###
        # obtain the server mac address and switch port from server_arp_table
        ...
        log.info("Packet from client %s forwarded to server %s." % (client_ip, server_ip))log.info("called function: forward_client_to_server")
